(function(root){
  ...
  function segmentDistance(x,A,B,out){var a=A.a*3,b=A.b*3,c=B.a*3,d=B.b*3,ux=x[b]-x[a],uy=x[b+1]-x[a+1],uz=x[b+2]-x[a+2],vx=x[d]-x[c],vy=x[d+1]-x[c+1],vz=x[d+2]-x[c+2],wx=x[a]-x[c],wy=x[a+1]-x[c+1],wz=x[a+2]-x[c+2],aa=ux*ux+uy*uy+uz*uz,bb=ux*vx+uy*vy+uz*vz,cc=vx*vx+vy*vy+vz*vz,dd=ux*wx+uy*wy+uz*wz,ee=vx*wx+vy*wy+vz*wz,den=aa*cc-bb*bb,s=den>1e-20?clamp((bb*ee-cc*dd)/den,0,1):0,t=(bb*s+ee)/(cc||1);
    ...
    out=out||{};out.distance=Math.hypot(dx,dy,dz);out.s=s;out.t=t;out.dx=dx;out.dy=dy;out.dz=dz;return out;
  }
  function edgeBox(x,e){var a=e.a*3,b=e.b*3;return [Math.min(x[a],x[b]),Math.min(x[a+1],x[b+1]),Math.min(x[a+2],x[b+2]),Math.max(x[a],x[b]),Math.max(x[a+1],x[b+1]),Math.max(x[a+2],x[b+2])];}
  function boxDistance(A,B){var dx=Math.max(0,A[0]-B[3],B[0]-A[3]),dy=Math.max(0,A[1]-B[4],B[1]-A[4]),dz=Math.max(0,A[2]-B[5],B[2]-A[5]);return Math.hypot(dx,dy,dz);}
  function evaluate(x,model,factor,p,gradient){var n=x.length,g=gradient?new Float64Array(n):null,E=0,edges=model.edges,scratch=model.evalScratch||(model.evalScratch={len:new Float64Array(edges.length),tx:new Float64Array(edges.length),ty:new Float64Array(edges.length),tz:new Float64Array(edges.length),close:{}}),len=scratch.len,tx=scratch.tx,ty=scratch.ty,tz=scratch.tz,rp=2e-5*p.repulsiveForce,minGap=Infinity,boundaryGap=Infinity;
    ...
    var pairs=model.pairs;for(var pi=0;pi<pairs.length;pi++){var pair=pairs[pi],i=pair.i,j=pair.j,A=edges[i],B=edges[j];
      var close=segmentDistance(x,A,B,scratch.close),gap=close.distance-1e-5,isLocal=pair.local;if(!isLocal){minGap=Math.min(minGap,gap);if(gap<=0)return {energy:Infinity,gradient:g,minGap:minGap,boundaryGap:boundaryGap};}
      var delta=.018,weight=pair.weight;
  ...
  function displayCurve(model,result){var x=[],edges=[],loops=[],radii=[],source=result.x,sub=globalThis.TypeDeformerDraftMode?2:4;
    model.loops.forEach(function(loop,li){var start=x.length/3,count=loop.count*sub;loops.push({start:start,count:count,radius:loop.radius});
      for(var j=0;j<loop.count;j++){var a=(loop.start+(j+loop.count-1)%loop.count)*3,b=(loop.start+j)*3,c=(loop.start+(j+1)%loop.count)*3,rr=Math.min(model.edges[loop.start+j].radius,model.edges[loop.start+(j+loop.count-1)%loop.count].radius);
        for(var k=0;k<sub;k++){var t=k/(sub-1),pt=[];for(var axis=0;axis<3;axis++){var P=source[b+axis]*.73+source[a+axis]*.27,Q=source[b+axis]*.73+source[c+axis]*.27;pt.push(P*(1-t)*(1-t)+2*source[b+axis]*t*(1-t)+Q*t*t);}var field=fieldAt(model.domain,pt[0],pt[1]);for(var repair=0;repair<12&&field.value<.0001;repair++){for(var axis=0;axis<3;axis++)pt[axis]=(pt[axis]+source[b+axis])*.5;field=fieldAt(model.domain,pt[0],pt[1]);}x.push(...pt);radii.push(Math.max(1e-7,Math.min(rr,field.value*.8,(model.halfDepth-Math.abs(pt[2]))*.8)));}
      }
  ...
  function tubeMesh(model,result){var mesh={v:[],t:[],diagnostics:result.diagnostics},smooth=displayCurve(model,result);model=smooth;var x=smooth.x,sides=globalThis.TypeDeformerDraftMode?4:8;
    var radii=model.edges.map(function(e){return e.radius;}),boxes=model.edges.map(function(e){return edgeBox(x,e);});
    for(var i=0;i<model.edges.length;i++)for(var j=i+1;j<model.edges.length;j++)if(!localPair(model.edges[i],model.edges[j],model)){if(.4*boxDistance(boxes[i],boxes[j])>=Math.max(radii[i],radii[j]))continue;var cap=.4*segmentDistance(x,model.edges[i],model.edges[j]).distance;radii[i]=Math.min(radii[i],cap);radii[j]=Math.min(radii[j],cap);}
  ...
  var cache=new Map(),domainCache=new Map(),modelCache=new Map(),cacheStats={domainHits:0,domainMisses:0,modelHits:0,modelMisses:0,resultHits:0,resultMisses:0};
  function indexPairs(model){var pairs=[];for(var i=0;i<model.edges.length;i++)for(var j=i+1;j<model.edges.length;j++){var A=model.edges[i],B=model.edges[j];if(A.a===B.a||A.a===B.b||A.b===B.a||A.b===B.b)continue;pairs.push({i:i,j:j,local:localPair(A,B,model),weight:.2*Math.sqrt(A.rest*B.rest)});}model.pairs=pairs;return model;}
  function prepareModel(domain,p,seed){var model=indexPairs(initialize(domain,p,seed));model.initialization='contours';if(!Number.isFinite(evaluate(model.x,model,1,p,false).energy)){model=indexPairs(initializeDisks(domain,p,seed));model.initialization='interior rings';}return model;}
  function construct(s,p,seed){var draft=!!globalThis.TypeDeformerDraftMode,resolution=draft?32:144,domainKey=[draft?'draft':'exact',s.sourceKey||s.key,s.w,s.h,resolution].join('|'),domain=domainCache.get(domainKey);if(domain){cacheStats.domainHits++;domainCache.delete(domainKey);domainCache.set(domainKey,domain);}else{cacheStats.domainMisses++;domain=domainFromSource(s,resolution);if(domainCache.size>=24)domainCache.delete(domainCache.keys().next().value);domainCache.set(domainKey,domain);}if(domain.empty)return {empty:true,domain:domain};seed=seed==null?17:seed;var modelKey=[domainKey,p.repulsiveWidth,p.repulsiveDepth,p.repulsiveStrands,seed].join('|'),model=modelCache.get(modelKey);if(model){cacheStats.modelHits++;modelCache.delete(modelKey);modelCache.set(modelKey,model);}else{cacheStats.modelMisses++;model=prepareModel(domain,p,seed);if(modelCache.size>=16)modelCache.delete(modelCache.keys().next().value);modelCache.set(modelKey,model);}var key=[modelKey,p.repulsiveLength,p.repulsiveForce,p.repulsiveBend].join('|'),hit=cache.get(key);if(hit){cacheStats.resultHits++;cache.delete(key);cache.set(key,hit);return {domain:domain,model:hit.model,result:hit.result,cached:true};}cacheStats.resultMisses++;var result=minimize(model,p,draft?4:170);result.diagnostics.initialization=model.initialization;if(!result.initialValid)return {empty:true,domain:domain,failure:result.diagnostics};if(cache.size>=12)cache.delete(cache.keys().next().value);cache.set(key,{model:model,result:result});return {domain:domain,model:model,result:result,cached:false};}
  ...
  root.TypeDeformerRepulsiveCurves={schemas:{repulsiveCurves:schema},ids:['repulsiveCurves'],renderers:{repulsiveCurves:render},effectPad:function(id,g){return Math.ceil((g?Math.max(g.w,g.h):180)*1.1+12);},cacheStats:function(){return Object.assign({domains:domainCache.size,models:modelCache.size,results:cache.size},cacheStats);},internals:{distanceTransform:distanceTransform,domainFromSource:domainFromSource,fieldAt:fieldAt,contourLoops:contourLoops,resample:resample,initialize:initialize,indexPairs:indexPairs,prepareModel:prepareModel,segmentDistance:segmentDistance,edgeBox:edgeBox,boxDistance:boxDistance,localPair:localPair,evaluate:evaluate,minimize:minimize,displayCurve:displayCurve,tubeMesh:tubeMesh,construct:construct}};
