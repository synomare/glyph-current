(function(root){
  ...
  function make(options){options=options||{};var purpose=['edit','proof','export'].includes(options.purpose)?options.purpose:'edit';
    var c={purpose:purpose,presentation:options.presentation==='low'&&purpose==='edit'?'low':'standard',
      ...
      precision:purpose==='edit'?'display':'output',memoryBudget:options.memoryBudget||512*MiB,
      maxCanvasPixels:options.maxCanvasPixels||96*MiB,tile:options.tile||null,
  ...
  function release(canvas){var m=metadata.get(canvas),contexts=[];
    if(current)contexts.push(current);if(m&&m.context&&m.context!==current)contexts.push(m.context);
    contexts.forEach(function(context){var bytes=context.accounted.get(canvas)||0;context.bytes=Math.max(0,context.bytes-bytes);context.accounted.delete(canvas);});
