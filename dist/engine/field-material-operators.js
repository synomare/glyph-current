(function (root) {
  ...
  var sourceCache=new Map(),sourceCacheBytes=0,SOURCE_MAX_BYTES=48*1024*1024,cacheStats={sourceHits:0,sourceMisses:0,outputHits:0,outputMisses:0};
  ...
  function source(g,L,fm,env,id,p,padOverride){
    var scale=globalThis.TypeDeformerDraftMode?1:2, pad=padOverride==null?requestedPad(id,g,p):Math.ceil(padOverride);
    ...
    var sourceKey=[globalThis.TypeDeformerWorkerFontRevision||0,id,g.ch,g.fontFamily,g.fontWeight,JSON.stringify(g.fontAxes||null),g.w,g.h,inkX-g.x,inkY-g.y,inkW,inkH,worldW,worldH,pad,scale,fm&&fm.ascent,fm&&fm.descent].join('|');
    var tile;if(id==='repulsiveCurves'){var sourceCanvas=document.createElement('canvas');sourceCanvas.width=w;sourceCanvas.height=h;tile={canvas:sourceCanvas,ctx:sourceCanvas.getContext('2d',{willReadFrequently:true})};}else tile=env.scratch('field-material-source',w,h,true);
    var local=Object.assign({},g,{x:g.x-x,y:g.y-y,ox:0,oy:0,tx:0,ty:0,rot:0,skewX:0,skewY:0,scaleX:1,scaleY:1,opacity:1}),stored=sourceCache.get(sourceKey),data,alpha,hash;
    env.drawGlyph(tile.ctx,local,scale,{dx:0,dy:0,s:1},fm,1,'#ffffff');
    if(stored&&stored.w===w&&stored.h===h){sourceCache.delete(sourceKey);sourceCache.set(sourceKey,stored);cacheStats.sourceHits++;data=stored.data;alpha=stored.alpha;hash=stored.key;}
    else{
      cacheStats.sourceMisses++;data=tile.ctx.getImageData(0,0,w,h).data;alpha=new Float32Array(w*h);
      for(var i=0;i<alpha.length;i++)alpha[i]=data[i*4+3]/255;hash=fingerprint(data);
      var bytes=data.byteLength+alpha.byteLength;while(sourceCacheBytes+bytes>SOURCE_MAX_BYTES&&sourceCache.size){var oldest=sourceCache.keys().next().value;sourceCacheBytes-=sourceCache.get(oldest).bytes;sourceCache.delete(oldest);}if(bytes<=SOURCE_MAX_BYTES){stored={data:new Uint8ClampedArray(data),alpha:new Float32Array(alpha),key:hash,w:w,h:h,bytes:bytes};sourceCache.set(sourceKey,stored);sourceCacheBytes+=bytes;}
    }
    var result={canvas:tile.canvas,data:data,alpha:alpha,w:w,h:h,scale:scale,pad:pad,x:x,y:y,key:hash,sourceKey:sourceKey,trace:env.traceContours};
  ...
  function touchesRasterEdge(c,gutter){var w=c.width,h=c.height;if(!w||!h)return false;var ctx=c.getContext('2d'),g=Math.max(1,Math.min(gutter||3,Math.floor(Math.min(w,h)/2))),strips=[ctx.getImageData(0,0,w,g).data,ctx.getImageData(0,h-g,w,g).data];if(h>g*2)strips.push(ctx.getImageData(0,g,g,h-g*2).data,ctx.getImageData(w-g,g,g,h-g*2).data);for(var n=0;n<strips.length;n++)for(var i=3;i<strips[n].length;i+=4)if(strips[n][i]>1)return true;return false;}
  ...
  function entryFor(id,g,L,fm,env){
    ...
    for(var attempt=0;attempt<4;attempt++){
      s=source(g,L,fm,env,id,p,pad);key=[id,s.w,s.h,s.key,JSON.stringify(p),color,accent,seed,root.TypeDeformerRenderContext&&root.TypeDeformerRenderContext.current()?root.TypeDeformerRenderContext.current().factor:1,root.TypeDeformerRenderContext&&root.TypeDeformerRenderContext.current()?root.TypeDeformerRenderContext.current().purpose:'analysis'].join('|');entry=cache.get(key);
      if(entry){cacheStats.outputHits++;cache.delete(key);cache.set(key,entry);break;}cacheStats.outputMisses++;
  ...
  var fieldApi=root.TypeDeformerFieldMaterials,clearOutputCache=fieldApi.clearCache,outputCacheStats=fieldApi.cacheStats;
  fieldApi.clearCache=function(){clearOutputCache();sourceCache.clear();sourceCacheBytes=0;cacheStats={sourceHits:0,sourceMisses:0,outputHits:0,outputMisses:0};};
  fieldApi.cacheStats=function(){return Object.assign({},outputCacheStats(),{sourceEntries:sourceCache.size,sourceBytes:sourceCacheBytes,sourceMaxBytes:SOURCE_MAX_BYTES,hits:Object.assign({},cacheStats)});};
